...
| 路径 | 职责摘要 | 本地文档 |
|------|----------|----------|
| `app/` | 应用包总览与子模块导航 | [app/AGENTS.md](app/AGENTS.md) |
| `app/api/` | FastAPI 路由：chat、memory、sessions、agents、OpenAI 兼容、诊断、工具等 | [app/api/AGENTS.md](app/api/AGENTS.md) |
| `app/agents/` | Planner / Executor / Workflow、run 存储 | [app/agents/AGENTS.md](app/agents/AGENTS.md) |
| `app/core/` | Orchestrator、ModelRouter、会话身份、错误与请求上下文、诊断 | [app/core/AGENTS.md](app/core/AGENTS.md) |
| `app/db/` | `database`、SQLAlchemy 模型、版本化迁移 | [app/db/AGENTS.md](app/db/AGENTS.md) |
| `app/memory/` | 检索、向量库、嵌入、Obsidian、记忆流水线、检索质量 | [app/memory/AGENTS.md](app/memory/AGENTS.md) |
| `app/tools/` | 工具注册、shell/file/git 等实现与审计 | [app/tools/AGENTS.md](app/tools/AGENTS.md) |
| `app/scheduler/` | APScheduler 集成与 jobs | [app/scheduler/AGENTS.md](app/scheduler/AGENTS.md) |
| `app/cli/` | 调用本地 HTTP API 的 Typer CLI | [app/cli/AGENTS.md](app/cli/AGENTS.md) |
| `tests/` | `pytest`，含 `integration` 标记 | [tests/AGENTS.md](tests/AGENTS.md) |
| `scripts/` | 迁移、运行时检查、检索评估等 | [scripts/AGENTS.md](scripts/AGENTS.md) |
| `docs/` | 路线图、测试与开源就绪等 | [docs/AGENTS.md](docs/AGENTS.md) |
...
- **已覆盖模块占比**：下表所列 **12** 个子路径均写入/更新 `AGENTS.md`，视为模块文档 **100%** 覆盖（按本模板定义的模块边界）。
