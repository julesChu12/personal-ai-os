...
Obsidian 双向同步已完成基础版：先 dry-run、再 apply，默认非破坏性删除，冲突只报告不自动合并。实时文件监听和自动冲突合并仍应作为后续独立阶段。
...
当前实现说明：
已提供轻量 SQLAlchemy migration runner、初始 schema migration、`scripts/run_migrations.py` 入口、`migration-smoke` CI 目标和文档。N10 后应用启动不再 `create_all`，而是校验 required tables；缺少 schema 会明确提示先运行 migration。Docker Compose 的 API 服务在启动 Uvicorn 前自动执行 migration。
...
| N11 Obsidian 双向同步基础版 | P2 | 已完成 | dry-run/apply、sync state、vault/DB 双向更新、冲突报告和默认非破坏性删除 |
| N8 真实 embedding 在线质量回归 | P2 | 已完成 | 基于 N1 口径支持手动或定时真实 provider 质量评估 |
| N9 CLI 升级 | P3 | 已完成 | 增加 `agents` 子命令、JSON 输出、错误码和最近 run 查询 |
| N10 移除 `create_all` 兼容路径 | P3 | 已完成 | migration 成为 schema 单一真实来源 |
...
### Obsidian 实时监听与自动冲突合并

暂缓原因：
当前已完成手动/脚本触发的双向同步基础版。实时文件监听、自动冲突合并和破坏性删除传播仍会显著增加误删/覆盖风险，应独立做 spec 和 UAT。
